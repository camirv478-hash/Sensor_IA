...
class ApiService {
  ...
  // (respaldo por si flutter_secure_storage falla en web)
  // ============================================
  ...
  Future<Map<String, String>> _headers({bool isMultipart = false}) async {
    ...
    // Cabecera obligatoria para evitar la página de advertencia de ngrok
    final Map<String, String> headers = {
      'ngrok-skip-browser-warning': 'true',
    };

    // Si es multipart, no incluimos Content-Type (lo maneja http.MultipartRequest)
    if (isMultipart) {
      if (token != null) {
  ...
  Future<bool> login(String username, String password) async {
    try {
      final response = await http.post(
        ...
        headers: await _headers(),   // <-- AHORA USA LAS CABECERAS CORRECTAS (con ngrok-skip)
